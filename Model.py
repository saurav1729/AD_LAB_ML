...
class GraphCNN(nn.Module):
    ...
    def forward(self, x_market,x_news,edge_list,inter_metric,device1):
        ...
        edge_index1 = edge_index1.to(device1)
        ...
        d[d<1]=0
        c = torch.from_numpy(d).to(device1)
        c_adj= self.attentions(x_s, c)
        edge_index+=c_adj.tolist()
        company_list=c_adj.tolist()
        edge_type+=[torch.ones(len(company_list)) * 4]
        edge_type = torch.cat(edge_type, dim=0).to(device1)
        c_graph = [x_s, torch.LongTensor(edge_index).transpose(0, 1).to(device1), edge_type]
        edge_index = []
        edge_dict = {}
        edge_type = []
        ...
        for eg in ['FS', 'FE']:
            ...
            edge_type += [torch.ones(len(edge_list[eg])) * edge_dict[eg]]
        edge_type = torch.cat(edge_type, dim=0).to(device1)
        t_graph = [torch.LongTensor(edge_index).transpose(0, 1).to(device1), edge_type]

        edge_index = []
        edge_dict = {}
        edge_type = []
        ## the explicit executives relations
        for eg in ['R', 'C', 'CL']:
            if eg not in edge_dict:
                edge_dict[eg] = len(edge_dict)
            edge_index += edge_list[eg]
            edge_type += [torch.ones(len(edge_list[eg])) * edge_dict[eg]]
        edge_type = torch.cat(edge_type, dim=0).to(device1)
        p_graph = [x, torch.LongTensor(edge_index).transpose(0, 1).to(device1), edge_type]
